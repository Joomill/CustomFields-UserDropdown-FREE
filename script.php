...
class plgFieldsUserdropdownInstallerScript
{
	...
	public function postflight($type, $parent)
	{
		if ($type === 'install')
		{
			...
			echo '<br>';
			echo '<h3 class="text-center">' . Text::_('PLG_FIELDS_USERDROPDOWN_THANKYOU') . '</h3>';
			echo '<br>';
			echo '<div class="text-center">' . Text::_('PLG_FIELDS_USERDROPDOWN_FOLLOWME') . ':</div>';
	...
	private function enablePlugin()
	{
		try
		{
			...
			$query = $db->getQuery(true)
				...
				->where('element = ' . $db->q('userdropdown'));
			$db->setQuery($query);
			$db->execute();
		}
		catch (\Exception $e)
